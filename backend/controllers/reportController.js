...
const getDashboardStats = async (req, res, next) => {
    try {
        ...
        res.json({
            ...
            netProfit: (monthlySales[0]?.total || 0) - (monthlyExpenses[0]?.total || 0),
...
const getPnLReport = async (req, res, next) => {
    try {
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            const e = expenses.find(x => x._id.month === month && x._id.year === year);
    ...
    try {
        ...
        const formatted = gstData.map(item => {
            ...
            return {
                ...
                totalWithTax: Math.round((item.taxableAmount + item.totalTax) * 100) / 100,
        ...
        const productSales = await Invoice.aggregate([
            { $unwind: '$items' },
            {
                $group: {
                    ...
                    totalTax: { $sum: { $multiply: ['$items.price', '$items.quantity', '$items.taxRate'] } },
                    orderCount: { $sum: 1 }
                }
            },
            { $sort: { totalRevenue: -1 } },
...
const getLowStockReport = async (req, res, next) => {
    try {
        ...
        }).select('name sku currentStock minStockLevel maxStockLevel category price').sort({ currentStock: 1 });
...
const getCustomerOutstandingReport = async (req, res, next) => {
    try {
        ...
        const linkedOutstanding = await Invoice.aggregate([
            { $match: { status: { $in: ['UNPAID', 'PARTIAL'] }, customerId: { $ne: null } } },
            {
                $group: {
                    _id: '$customerId',
