  ...
  try {
    ...
    if (customer) {
      ...
      if (req.body.outstandingBalance !== undefined)
        customer.outstandingBalance = req.body.outstandingBalance;
...
const getCustomerLedger = async (req, res, next) => {
  try {
    ...
    const invoicesWithType = invoices.map((inv) => ({
      ...inv,
      type: "INVOICE",
      date: inv.createdAt,
    }));
    const paymentsWithType = payments.map((pay) => ({
      ...pay,
      type: "PAYMENT",
    }));

    // Merge and sort by date descending
    const transactions = [...invoicesWithType, ...paymentsWithType].sort(
