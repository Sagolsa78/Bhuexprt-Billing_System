  ...
  try {
    ...
    if (customerId) {
      ...
      if (customer.creditLimit > 0 && newBalance > customer.creditLimit) {
        res.status(400);
        throw new Error(
          `Credit limit exceeded. Current Balance: ${customer.outstandingBalance}, New Total: ${newBalance}, Limit: ${customer.creditLimit}`,
        );
      }
  ...
  try {
    ...
    const result = invoices.map((inv) => {
      ...
      if (
        ...
        invoiceObj.customerId &&
        invoiceObj.customerId.name
      ) {
        ...
        updatePromises.push(
          Invoice.findByIdAndUpdate(inv._id, {
            ...
            customerEmail:
              invoiceObj.customerEmail || invoiceObj.customerId.email || "",
            customerMobile:
              invoiceObj.customerMobile || invoiceObj.customerId.phone || "",
            customerAddress:
              invoiceObj.customerAddress || invoiceObj.customerId.address || "",
          }),
        );
        // Also set it in the response
        invoiceObj.customerName = invoiceObj.customerId.name;
        if (!invoiceObj.customerEmail)
          invoiceObj.customerEmail = invoiceObj.customerId.email || "";
        if (!invoiceObj.customerMobile)
          invoiceObj.customerMobile = invoiceObj.customerId.phone || "";
        if (!invoiceObj.customerAddress)
          invoiceObj.customerAddress = invoiceObj.customerId.address || "";
      }
      return invoiceObj;
    });
