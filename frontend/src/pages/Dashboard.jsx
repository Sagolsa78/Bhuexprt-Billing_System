...
const Dashboard = () => {
  ...
  const totalRevenue =
    stats?.totalSales ||
    invoices.reduce(
      (acc, inv) => acc + (inv.status === "PAID" ? inv.total : 0),
      0,
    );
  const pendingAmount =
    stats?.totalReceivables ||
    invoices.reduce(
      (acc, inv) => acc + (inv.status === "UNPAID" ? inv.total : 0),
      0,
    );
    ...
    <div className="p-2 sm:p-4">
      ...
      relative overflow-hidden group`}
            >
              {/* Top Accent Line */}
              <div
                className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${themeColors.gradient}`}
              />

              <div className="p-5 flex items-center">
                <div
                  className={`flex-shrink-0 ${themeColors.bgLight} dark:bg-gray-700 rounded-xl p-3`}
                >
